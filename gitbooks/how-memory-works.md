...
Traditional AI memory systems store everything and retrieve by similarity. But the human brain works differently — it actively forgets. Psychologist Hermann Ebbinghaus showed that memories decay exponentially over time unless reinforced. This isn't a flaw; it's a feature. Forgetting keeps your mental model lean, relevant, and fast.
...
The brain doesn't store raw sensory data — it compresses experiences into meaningful patterns and discards the rest. You remember the gist of a conversation, not every word.
...
The brain organizes knowledge as a web of associations — people, places, events, and concepts linked by relationships. Remembering one thing triggers related memories.

Neocortex uses a **Graph-based Retrieval-Augmented Generation (GraphRAG)** model. Documents are broken into chunks, and an LLM extracts **entities** (people, companies, products) and **relations** (founded, works at, invested in) to build a knowledge graph. Queries traverse this graph to retrieve contextually rich, structured information — not just similar text snippets.
...
| Focused, lean recall | Automatic cleanup — no manual pruning |
